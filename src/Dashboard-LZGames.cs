...
namespace LZGames.UI {
    public static class Theme {
        public static readonly Color Background=Color.FromArgb(14,17,23), Surface=Color.FromArgb(22,26,34), Raised=Color.FromArgb(30,36,46), Border=Color.FromArgb(48,56,68), Text=Color.FromArgb(239,244,248), Muted=Color.FromArgb(156,168,184), Green=Color.FromArgb(105,227,189), Violet=Color.FromArgb(124,158,238);
        ...
        public static Color Mix(Color a,Color b,float amount){float t=Math.Max(0,Math.Min(1,amount));return Color.FromArgb((int)(a.R+(b.R-a.R)*t),(int)(a.G+(b.G-a.G)*t),(int)(a.B+(b.B-a.B)*t));}
    ...
    public class StudioLayout : TableLayoutPanel {
        public StudioLayout(){SetStyle(ControlStyles.AllPaintingInWmPaint|ControlStyles.OptimizedDoubleBuffer|ControlStyles.ResizeRedraw,true);}
        protected override void OnSizeChanged(EventArgs e){base.OnSizeChanged(e);Invalidate(true);}
    }
    public class WorkspacePane : Panel {
        public WorkspacePane(){DoubleBuffered=true;ResizeRedraw=true;BackColor=Theme.Surface;}
        protected override void OnSizeChanged(EventArgs e){base.OnSizeChanged(e);Invalidate(true);}
    }
    public class TabStrip : StudioLayout {
        private int selected;
        private float underline;
        private readonly Timer transition;
        public TabStrip(){transition=new Timer{Interval=16};transition.Tick+=(s,e)=>Step();}
        public void Select(int index){selected=index;if(!Visible){Snap();return;}transition.Start();Invalidate();}
        private float Target {get{return selected*Width/3f;}}
        private void Step(){underline+=(Target-underline)*0.28f;if(Math.Abs(Target-underline)<0.5f){underline=Target;transition.Stop();}Invalidate();}
        private void Snap(){underline=Target;if(transition!=null)transition.Stop();Invalidate();}
        protected override void OnSizeChanged(EventArgs e){base.OnSizeChanged(e);Snap();}
        protected override void OnVisibleChanged(EventArgs e){base.OnVisibleChanged(e);if(!Visible)Snap();}
        protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);if(Width<3||Height<3)return;using(var p=new Pen(Theme.Border))e.Graphics.DrawLine(p,0,Height-1,Width,Height-1);using(var b=new SolidBrush(Theme.Green))e.Graphics.FillRectangle(b,underline+8,Height-3,Math.Max(1,Width/3f-16),2);}
        protected override void Dispose(bool disposing){if(disposing&&transition!=null)transition.Dispose();base.Dispose(disposing);}
    }
    public class Card : Panel {
        public Color Accent=Theme.Green;
        public bool Highlight=false;
        public Card() { SetStyle(ControlStyles.UserPaint|ControlStyles.AllPaintingInWmPaint|ControlStyles.OptimizedDoubleBuffer|ControlStyles.ResizeRedraw,true); BackColor=Theme.Surface; Padding=new Padding(18); }
        protected override void OnSizeChanged(EventArgs e){base.OnSizeChanged(e);Invalidate(true);}
        protected override void OnPaintBackground(PaintEventArgs e){
            e.Graphics.Clear(Theme.ParentColor(this));
            if(Width<3||Height<3)return;
            e.Graphics.SmoothingMode=SmoothingMode.AntiAlias;
            using(var p=Theme.Round(new Rectangle(0,0,Width-1,Height-1),12))using(var b=new SolidBrush(BackColor))e.Graphics.FillPath(b,p);
        }
    ...
    public class StudioMark : Control {
        public StudioMark(){DoubleBuffered=true;ResizeRedraw=true;BackColor=Theme.Background;}
        protected override void OnPaint(PaintEventArgs e){
            var g=e.Graphics;g.SmoothingMode=SmoothingMode.AntiAlias;
            int s=Math.Min(Width,Height)-6;if(s<8)return;var r=new Rectangle((Width-s)/2,(Height-s)/2,s,s);
            using(var p=Theme.Round(r,12)){
                using(var b=new SolidBrush(Theme.Green))g.FillPath(b,p);
            }
            using(var f=Theme.Font(17,true))TextRenderer.DrawText(g,"LZ",f,r,Theme.Background,TextFormatFlags.HorizontalCenter|TextFormatFlags.VerticalCenter);
        }
    }
    public class StudioHeader : Panel {
        public StudioHeader(){DoubleBuffered=true;ResizeRedraw=true;BackColor=Theme.Background;}
        protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);if(Width<2||Height<2)return;
            using(var p=new Pen(Theme.Border))e.Graphics.DrawLine(p,0,Height-1,Width,Height-1);
        }
    }
    public class EmptyLibrary : Panel {
        public EmptyLibrary(){DoubleBuffered=true;ResizeRedraw=true;BackColor=Theme.Surface;}
        protected override void OnPaint(PaintEventArgs e){
            base.OnPaint(e);var g=e.Graphics;g.SmoothingMode=SmoothingMode.AntiAlias;
            if(Height<225){
                int top=Math.Max(0,(Height-54)/2-28);
                using(var f=Theme.Font(14,true))TextRenderer.DrawText(g,"Sua biblioteca começa aqui",f,new Rectangle(0,top,Width,28),Theme.Text,TextFormatFlags.HorizontalCenter|TextFormatFlags.EndEllipsis);
                using(var f=Theme.Font(9))TextRenderer.DrawText(g,"Selecione a pasta principal dos vídeos.",f,new Rectangle(0,top+30,Width,22),Theme.Muted,TextFormatFlags.HorizontalCenter|TextFormatFlags.EndEllipsis);
                return;
            }
            int cx=Width/2,cy=Math.Max(48,(Height-54)/2-40);
            var state=g.Save();g.TranslateTransform(cx,cy);
            using(var pen=new Pen(Theme.Border)){
                using(var p=Theme.Round(new Rectangle(-68,-27,86,58),8))g.DrawPath(pen,p);
                using(var p=Theme.Round(new Rectangle(-18,-27,86,58),8))g.DrawPath(pen,p);
            }
            using(var p=Theme.Round(new Rectangle(-41,-40,82,70),10)){
                using(var b=new LinearGradientBrush(new Rectangle(-41,-40,82,70),Color.FromArgb(36,55,54),Theme.Raised,90f))g.FillPath(b,p);
                using(var pen=new Pen(Color.FromArgb(77,117,108)))g.DrawPath(pen,p);
            }
            using(var b=new SolidBrush(Theme.Green))g.FillPolygon(b,new Point[]{new Point(-7,-18),new Point(13,-5),new Point(-7,8)});
            g.Restore(state);
            using(var f=Theme.Font(19,true))TextRenderer.DrawText(g,"Sua biblioteca começa aqui",f,new Rectangle(0,cy+46,Width,36),Theme.Text,TextFormatFlags.HorizontalCenter|TextFormatFlags.EndEllipsis);
            using(var f=Theme.Font(10))TextRenderer.DrawText(g,"Selecione uma pasta. Nós organizamos a fila e suas subpastas.",f,new Rectangle(0,cy+88,Width,24),Theme.Muted,TextFormatFlags.HorizontalCenter|TextFormatFlags.EndEllipsis);
        }
    }
    public class ActionButton : Button {
        public bool Primary=false, Selected=false, Segment=false;
        private bool hover,pressed;
        private float hoverAmount;
        private readonly Timer hoverAnimation;
        public ActionButton() { SetStyle(ControlStyles.UserPaint|ControlStyles.AllPaintingInWmPaint|ControlStyles.OptimizedDoubleBuffer|ControlStyles.ResizeRedraw,true); FlatStyle=FlatStyle.Flat;FlatAppearance.BorderSize=0;Cursor=Cursors.Hand;Font=Theme.Font(10,true);ForeColor=Theme.Text;BackColor=Theme.Surface;Margin=new Padding(4);hoverAnimation=new Timer{Interval=16};hoverAnimation.Tick+=(s,e)=>StepHover(); }
        private void StepHover(){float target=hover&&Enabled?1:0;hoverAmount+=Math.Sign(target-hoverAmount)*0.125f;if(Math.Abs(target-hoverAmount)<0.126f){hoverAmount=target;hoverAnimation.Stop();}Invalidate();}
        protected override void OnMouseEnter(EventArgs e){hover=true;if(Enabled)hoverAnimation.Start();base.OnMouseEnter(e);}
        protected override void OnMouseLeave(EventArgs e){hover=false;pressed=false;hoverAnimation.Start();base.OnMouseLeave(e);}
        ...
        protected override void OnVisibleChanged(EventArgs e){base.OnVisibleChanged(e);if(!Visible&&hoverAnimation!=null){hoverAnimation.Stop();hover=false;hoverAmount=0;pressed=false;}}
        protected override void OnEnabledChanged(EventArgs e){base.OnEnabledChanged(e);if(!Enabled&&hoverAnimation!=null){hoverAnimation.Stop();hover=false;hoverAmount=0;pressed=false;}Invalidate();}
        protected override void Dispose(bool disposing){if(disposing&&hoverAnimation!=null)hoverAnimation.Dispose();base.Dispose(disposing);}
        protected override void OnPaint(PaintEventArgs e){
            e.Graphics.Clear(Theme.ParentColor(this));e.Graphics.SmoothingMode=SmoothingMode.AntiAlias;
            Color fill=Primary?Theme.Mix(Theme.Green,Color.FromArgb(145,239,211),hoverAmount):Theme.Mix(Theme.Raised,Color.FromArgb(43,51,61),hoverAmount);
            if(!Enabled)fill=Color.FromArgb(28,38,49);
            if(Width<3||Height<3)return;
            if(!Segment)using(var path=Theme.Round(new Rectangle(1,1,Width-3,Height-3),7)){
                using(var brush=new SolidBrush(fill))e.Graphics.FillPath(brush,path);
                using(var pen=new Pen(Theme.Mix(Primary?Theme.Green:Theme.Border,Theme.Green,hoverAmount)))e.Graphics.DrawPath(pen,path);
            }
            ...
            var textBounds=ClientRectangle;if(pressed)textBounds.Offset(0,1);
            TextRenderer.DrawText(e.Graphics,Text,Font,textBounds,!Enabled?Color.FromArgb(92,109,129):(Primary?Theme.Background:(Selected?Theme.Green:Segment?Theme.Mix(Theme.Muted,Theme.Text,hoverAmount):ForeColor)),flags);
            if(Focused&&ShowFocusCues)ControlPaint.DrawFocusRectangle(e.Graphics,new Rectangle(6,6,Width-13,Height-13),Primary?Theme.Background:Theme.Text,fill);
        }
    }
    ...
    public class Toggle : CheckBox {
        ...
        private readonly Timer slide;
        public Toggle(){AutoSize=false;Height=34;Font=Theme.Font(10);ForeColor=Theme.Text;Cursor=Cursors.Hand;SetStyle(ControlStyles.UserPaint|ControlStyles.AllPaintingInWmPaint|ControlStyles.OptimizedDoubleBuffer|ControlStyles.ResizeRedraw,true);slide=new Timer{Interval=16};slide.Tick+=(s,e)=>StepSlide();}
        private void StepSlide(){float target=Checked?1:0;position+=(target-position)*0.3f;if(Math.Abs(target-position)<0.02f){position=target;slide.Stop();}Invalidate();}
        protected override void OnVisibleChanged(EventArgs e){base.OnVisibleChanged(e);if(!Visible&&slide!=null){slide.Stop();position=Checked?1:0;}}
        protected override void OnEnabledChanged(EventArgs e){base.OnEnabledChanged(e);if(!Enabled&&slide!=null){slide.Stop();position=Checked?1:0;}Invalidate();}
        protected override void Dispose(bool disposing){if(disposing&&slide!=null)slide.Dispose();base.Dispose(disposing);}
        protected override void OnPaint(PaintEventArgs e){
            e.Graphics.Clear(Theme.ParentColor(this));e.Graphics.SmoothingMode=SmoothingMode.AntiAlias;
            using(var p=Theme.Round(new Rectangle(0,7,36,20),10))using(var b=new SolidBrush(Theme.Mix(Theme.Border,Theme.Green,position)))e.Graphics.FillPath(b,p);
            using(var b=new SolidBrush(Theme.Mix(Theme.Muted,Theme.Background,position)))e.Graphics.FillEllipse(b,3+16*position,10,14,14);
        ...
        protected override void OnCheckedChanged(EventArgs e){base.OnCheckedChanged(e);if(slide!=null){if(Visible)slide.Start();else position=Checked?1:0;}Invalidate();}
    }
    public class ProgressTrack : Control {
        ...
        public ProgressTrack(){Height=6;SetStyle(ControlStyles.UserPaint|ControlStyles.AllPaintingInWmPaint|ControlStyles.OptimizedDoubleBuffer|ControlStyles.ResizeRedraw,true);animation=new Timer{Interval=50};animation.Tick+=(s,e)=>{shimmer=(shimmer+5)%Math.Max(1,Width);Invalidate();};}
    ...
    public class PathDisplay : Control {
        public PathDisplay(){Font=Theme.Font(9);ForeColor=Theme.Text;BackColor=Theme.Raised;Dock=DockStyle.Fill;Margin=new Padding(8,0,2,0);SetStyle(ControlStyles.UserPaint|ControlStyles.AllPaintingInWmPaint|ControlStyles.OptimizedDoubleBuffer|ControlStyles.ResizeRedraw,true);}
    ...
    public class Dashboard : Form {
        ...
        public Dashboard(){
            Text="LZ Games · Video Studio 1.5";BackColor=Theme.Background;ForeColor=Theme.Text;Font=Theme.Font(10);
            ...
            root.RowStyles.Add(new RowStyle(SizeType.Absolute,62));root.RowStyles.Add(new RowStyle(SizeType.Absolute,10));
            root.RowStyles.Add(new RowStyle(SizeType.Absolute,28));root.RowStyles.Add(new RowStyle(SizeType.Absolute,12));root.RowStyles.Add(new RowStyle(SizeType.Percent,100));Controls.Add(root);
            ...
            var header=Table(4,1);header.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,56));header.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,100));header.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,158));header.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,96));headerFrame.Controls.Add(header);
            header.Controls.Add(new StudioMark{Dock=DockStyle.Fill,Margin=new Padding(0,3,12,3)},0,0);
            var brand=Table(1,2);brand.RowStyles.Add(new RowStyle(SizeType.Absolute,29));brand.RowStyles.Add(new RowStyle(SizeType.Percent,100));
            brand.Controls.Add(Label("Video Studio",20,true,Theme.Text),0,0);brand.Controls.Add(Label("LZ GAMES   /   Compactar, recortar e organizar.",9,false,Theme.Muted),0,1);header.Controls.Add(brand,1,0);
            var localLabel=Label("●  Processamento local\n     Versão 1.5",9,false,Theme.Muted);header.Controls.Add(localLabel,2,0);
            helpAction=Button("Ajuda",false);helpAction.Margin=new Padding(8,7,0,7);header.Controls.Add(helpAction,3,0);
            var metrics=Table(4,1);metrics.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,180));metrics.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,210));metrics.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,210));metrics.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,100));
            metrics.Controls.Add(Metric("Na fila","0",out filesCardValue),0,0);metrics.Controls.Add(Metric("Original","0 B",out inputCardValue),1,0);metrics.Controls.Add(Metric("Redução","—",out savingCardValue),2,0);savingCardValue.ForeColor=Theme.Green;
            ...
            var library=new WorkspacePane{Dock=DockStyle.Fill,Margin=new Padding(0,0,16,0),Padding=new Padding(18,10,18,12)};body.Controls.Add(library,0,0);
            ...
            var listHead=Table(2,1);listHead.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,100));listHead.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,108));listHead.Controls.Add(Label("Fila de processamento",14,true,Theme.Text),0,0);refreshButton=Button("Atualizar",false);refreshButton.Font=Theme.Font(9);listHead.Controls.Add(refreshButton,1,0);listLayout.Controls.Add(listHead,0,0);
            ...
            empty=new EmptyLibrary{Dock=DockStyle.Fill};var importRow=Table(3,1);importRow.Dock=DockStyle.Bottom;importRow.Height=46;importRow.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,50));importRow.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,244));importRow.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,50));var addFolder=Button("+   Selecionar pasta de vídeos",false);addFolder.Margin=new Padding(0,2,0,4);addFolder.Click+=(s,e)=>{SelectTab(2);if(inputBrowseButton.Enabled)inputBrowseButton.PerformClick();};importRow.Controls.Add(addFolder,1,0);empty.Controls.Add(importRow);gridHost.Controls.Add(empty);empty.BringToFront();
            ...
            var settings=new WorkspacePane{Dock=DockStyle.Fill,Padding=new Padding(18,10,18,12),Margin=new Padding(0)};body.Controls.Add(settings,1,0);
            var settingsLayout=Table(1,4);settingsLayout.RowStyles.Add(new RowStyle(SizeType.Absolute,36));settingsLayout.RowStyles.Add(new RowStyle(SizeType.Absolute,40));settingsLayout.RowStyles.Add(new RowStyle(SizeType.Percent,100));settingsLayout.RowStyles.Add(new RowStyle(SizeType.Absolute,88));settings.Controls.Add(settingsLayout);
            settingsLayout.Controls.Add(Label("Configuração de saída",14,true,Theme.Text),0,0);
            tabStrip=new TabStrip{Dock=DockStyle.Fill,ColumnCount=3,RowCount=1,Margin=new Padding(0),BackColor=Theme.Surface};tabStrip.RowStyles.Add(new RowStyle(SizeType.Percent,100));for(int i=0;i<3;i++)tabStrip.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,33.333f));tabs=new ActionButton[3];string[] tabNames={"Compactar","Recorte","Pastas"};
            ...
            Resize+=(s,e)=>{AdaptLayout();Invalidate(true);};
            AdaptLayout();
        }
        ...
        private void AdaptLayout(){
            ...
            rootLayout.RowStyles[0].Height=(compact?56:62)*scale;
            rootLayout.RowStyles[1].Height=10*scale;
            rootLayout.RowStyles[2].Height=28*scale;
            rootLayout.RowStyles[3].Height=12*scale;
        ...
        private static Panel Metric(string caption,string value,out Label valueLabel){var t=Table(2,1);t.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,66));t.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,100));t.Controls.Add(Label(caption,9,false,Theme.Muted),0,0);valueLabel=Label(value,10,true,Theme.Text);valueLabel.AutoEllipsis=true;t.Controls.Add(valueLabel,1,0);return t;}
        ...
        public void SelectTab(int index){if(index<0||index>=3)throw new ArgumentOutOfRangeException("index");for(int i=0;i<3;i++){pages[i].Visible=i==index;tabs[i].Selected=i==index;tabs[i].Invalidate();}pages[index].BringToFront();tabStrip.Select(index);}
