...
$result = [ordered]@{
    TestVersion = '1.5.0'; StartedUtc = [DateTime]::UtcNow.ToString('o')
...
try {
    ...
    Assert-Test ([Diagnostics.FileVersionInfo]::GetVersionInfo($installedExe).FileVersion -eq '1.5.0.0') 'Installed executable reports file version 1.5.0.0.'
    ...
    Assert-Test ($result.SelfTestReport.Passed -eq $true -and $result.SelfTestReport.Version -eq '1.5.0') 'Runtime self-test JSON declares success for version 1.5.0.'
